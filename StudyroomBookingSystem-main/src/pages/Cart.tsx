...
export function Cart() {
  ...
  const unpaidOrders = useMemo(
    () => foodOrders.filter((o) => o.status === '待支付'),
    [foodOrders]
  );

  function checkout() {
    const order = placeFoodOrder(delivery);
    if (!order) {
      setToast('购物车是空的');
      return;
    }
  ...
  return (
    <>
          ...
          {unpaidOrders.map((o, i) => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                <span style={{ fontSize: '0.88rem' }}>{o.id}</span>
      ...
      {cart.length === 0 ? (
        ...
        <>
          {cart.map((line) => (
            <div key={line.product.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
              <div>
                ...
                <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                  ...
                  <input
                    ...
                    value={line.qty}
                    onChange={(e) =>
                      setCartQty(line.product.id, Number(e.target.value) || 1)
                    }
      ...
      {foodOrders.length > 0 && (
        <section style={{ marginTop: '1.25rem' }}>
          ...
          {foodOrders.slice(0, 5).map((o) => (
            <div key={o.id} className="card" style={{ fontSize: '0.88rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>{o.id}</span>
